...
**A) Cần tra kho hoặc ảnh thuốc:**
Thought: (ngắn) vì sao cần tra cứu
Action: graphrag_query **hoặc** pill_image_lookup
Action Input: <chuỗi tìm kiếm phù hợp tool>
Observation:
...
- **Định dạng máy đọc:** Không bọc Markdown `**` quanh các nhãn Thought / Action / Action Input / Final Answer. Ghi đúng `Action: graphrag_query` hoặc `Action: pill_image_lookup` và `Action Input:` trên các dòng riêng như mẫu A (câu hỏi dài vẫn copy nguyên vào Action Input được).
"""
...
def get_parse_retry_prompt(error_message: str) -> str:
    ...
    return (
        ...
        + (error_message or "")
        + "\nHãy trả lời lại **một** trong hai: (1) Action: graphrag_query hoặc pill_image_lookup + Action Input: ... + dòng Observation: trống "
        "hoặc (2) Final Answer: ...\n"
