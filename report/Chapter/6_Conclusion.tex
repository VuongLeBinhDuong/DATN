...
\begin{itemize}
    ...
    \item \textbf{Graph-First Hybrid Retrieval with Clinical NER}: A high-precision retrieval engine was implemented, routing queries directly to the Custom KG. The engine leverages a parallel lightweight SLM (\texttt{qwen2.5:1.5b-instruct}) for Clinical NER extraction to execute targeted Cypher Path Queries linking concepts, falling back to standard k-hop expansions on the Custom KG to optimize precision, while bypassing external Microsoft GraphRAG CLI wrappers to reduce execution latency.
    \item \textbf{Hybrid Parallel Intent Routing}: Built a layered gateway combining a 0ms regex fast-path for physiological validator checks with a parallel, lightweight SLM classifier. This setup offloads query routing decisions to a 1.5B model (achieving a 54\% latency reduction) and reserves Llama-3.1-8B exclusively for the ReAct loop.
    \item \textbf{Resilient ReAct Agent}: A robust agentic reasoning loop was built on top of a local Ollama LLM, reinforced with three defensive mechanisms — parse-retry prompting, iteration-1 recovery-forced Custom KG retrieval, and loop-guard signature tracking — achieving a 100\% meaningful-response rate on adversarial test queries.
